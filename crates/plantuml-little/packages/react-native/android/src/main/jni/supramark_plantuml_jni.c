/*
 * supramark_d2_jni.c — JNI bridge between Java SupramarkPlantumlModule
 * and the C ABI exported by libsupramark_plantuml_native.so.
 *
 * Function naming follows JNI mangling for class
 * com.supramark.plantumlnative.SupramarkPlantumlModule — static native methods
...
#define TAG "SupramarkPlantUMLJNI"
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, TAG, __VA_ARGS__)

JNIEXPORT jstring JNICALL
Java_com_supramark_plantumlnative_SupramarkPlantumlModule_nativeRender(
    JNIEnv *env, jclass cls, jstring jSource, jintArray jStatusOut)
{
    (void)cls;
    if (jSource == NULL) {
        if (jStatusOut != NULL) {
            jint err = SUPRAMARK_PLANTUML_ERR_NULL_INPUT;
    ...
    uint8_t *outBuf = NULL;
    size_t   outLen = 0;
    int status = supramark_plantuml_render(src, (size_t)srcLen, &outBuf, &outLen);
    ...
    if (status != SUPRAMARK_PLANTUML_OK) {
        LOGE("supramark_plantuml_render returned %d", status);
        return NULL;
    }
    ...
    if (cstr == NULL) {
        ...
        if (jStatusOut != NULL) {
            jint err = SUPRAMARK_PLANTUML_ERR_RENDER;
            (*env)->SetIntArrayRegion(env, jStatusOut, 0, 1, &err);
        }
        return NULL;
    }
...
JNIEXPORT jstring JNICALL
Java_com_supramark_plantumlnative_SupramarkPlantumlModule_nativeGetVersion(
