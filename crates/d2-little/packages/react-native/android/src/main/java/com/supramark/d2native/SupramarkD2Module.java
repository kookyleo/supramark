/*
 * SupramarkD2Module.java — RN bridge module for d2 native FFI.
 *
 * Loads libsupramark_d2_jni.so (the JNI shim, which in turn links
 * libsupramark_d2_native.so), dispatches render calls off the JS
...
public class SupramarkD2Module extends ReactContextBaseJavaModule {

    public static final String NAME = "SupramarkD2Native";
    ...
    static {
        // Loading the JNI shim transitively pulls in libsupramark_d2_native.so
    ...
    private final ExecutorService renderQueue =
        Executors.newSingleThreadExecutor(r -> {
            Thread t = new Thread(r, "supramark-d2-native-render");
            t.setDaemon(true);
            return t;
        });

    public SupramarkD2Module(ReactApplicationContext reactContext) {
        super(reactContext);
    }
        ...
        renderQueue.execute(() -> {
            try {
                ...
                if (svg == null || status[0] != OK) {
                    ...
                    promise.reject(code, "supramark_d2_render returned " + status[0]);
                    return;
                }
                promise.resolve(svg);
            } catch (Throwable t) {
    ...
    public void getVersion(final Promise promise) {
        try {
            ...
            if (v == null) {
                promise.reject("UNKNOWN", "supramark_d2_version returned NULL");
            } else {
                promise.resolve(v);
            }
        } catch (Throwable t) {
