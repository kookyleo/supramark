...
W.getComputedStyle = function(el, pseudo) { const cs = __origGetComputedStyle(el, pseudo); if (!cs||cs.__paddingShimmed) return cs; const o=cs.getPropertyValue.bind(cs); cs.getPropertyValue=(n)=>{const v=o(n);return v===''||v==null?'0px':v}; try{Object.defineProperty(cs,'__paddingShimmed',{value:true})}catch{}; return cs; };
...
// Before loading mermaid, patch graphlib to log graph.nodes() calls
// We'll intercept the mermaid bundle's graphlib
const mermaid = (await import('mermaid')).default;
mermaid.initialize({ startOnLoad: false, securityLevel: 'loose', handDrawnSeed: 1 });

// Now patch the graph constructor to intercept nodes()
// We need to find the graphlib Graph class in the bundle
// It's imported by the dagre chunk

// Let's try patching at the graphlib level
const glMod = await import('dagre-d3-es/src/graphlib/index.js');
const origNodes = glMod.Graph.prototype.nodes;
glMod.Graph.prototype.nodes = function() {
  const result = origNodes.call(this);
  // Only log for graphs that have nodes c, b, a (inner graph of A)
  if (result.length >= 3 && result.includes('c') && result.includes('b') && result.includes('a') && !result.includes('O')) {
    console.log('graph.nodes() called, returning:', result);
  }
  return result;
};
