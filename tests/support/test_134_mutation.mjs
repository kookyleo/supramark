...
// Use MutationObserver to track DOM insert order
const nodeInsertOrder = [];
  ...
  for (const m of mutations) {
    for (const node of m.addedNodes) {
      if (node.getAttribute) {
        ...
        if (id.includes('flowchart-') && (id.includes('-a-') || id.includes('-b-') || id.includes('-c-'))) {
          nodeInsertOrder.push(id);
        }
      }
    }
  }
...
const { svg } = await mermaid.render('test-134-mut', src);

console.log('Node insert order (a, b, c only):');
for (const id of nodeInsertOrder) {
  console.log(`  ${id}`);
}
