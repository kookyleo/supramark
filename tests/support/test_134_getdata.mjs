...
// Intercept the render function to log data4Layout
const origDraw = mermaid.flowRenderer?.draw;
const diagram = await mermaid.parse(readFileSync('/ext/mermaid/tests/ext_fixtures/cypress/flowchart/134.mmd', 'utf8'));

const db = diagram.db;
console.log('getSubGraphs:', db.getSubGraphs().map(s => ({ id: s.id, nodes: s.nodes, classes: s.classes })));
console.log('getVertices:', Array.from(db.getVertices().values()).map(v => ({ id: v.id, type: v.type })));
const data = db.getData();
console.log('getData nodes:');
for (const n of data.nodes) {
  console.log(`  id=${n.id} isGroup=${n.isGroup} parentId=${n.parentId}`);
}
