...
if(W.HTMLImageElement){Object.defineProperty(W.HTMLImageElement.prototype,'src',{configurable:true,set(_v){setTimeout(()=>{this.dispatchEvent&&this.dispatchEvent(new W.Event('error'));},0);},get(){return'';}});Object.defineProperty(W.HTMLImageElement.prototype,'complete',{configurable:true,get(){return true;}});if(typeof W.HTMLImageElement.prototype.decode!=='function')W.HTMLElement.prototype.decode=function(){return Promise.resolve();};}
...
const src = readFileSync('/ext/mermaid/tests/ext_fixtures/cypress/flowchart/135.mmd', 'utf8');

// Access flowDb through render
const diagram = await mermaid.parse(src);
const db = diagram.db ?? diagram._db ?? diagram;
const vertices = db.getVertices?.() ?? [];
console.log('getVertices() order:');
let i = 0;
for (const [key, val] of vertices) {
  console.log(`  ${i}: ${key} domId=${val.domId}`);
  i++;
}
