...
// Minimal font shims (text metrics)
...
// Monkey-patch the layout function to intercept the inner graph
const mermaid = (await import('mermaid')).default;
mermaid.initialize({ startOnLoad: false, securityLevel: 'loose', handDrawnSeed: 1 });

// Access flowDb directly via parse
const src = readFileSync('/ext/mermaid/tests/ext_fixtures/cypress/flowchart/134.mmd', 'utf8');

// Parse first to get the diagram object
const diagram = await mermaid.parse(src);

// Now render with intercepted layout
const origConsoleWarn = console.warn;
const graphLogs = [];
console.warn = (...args) => {
  const msg = args.join(' ');
  if (msg.includes('nodes()') || msg.includes('children(') || msg.includes('Extracting node')) {
    graphLogs.push(msg);
  }
  origConsoleWarn.apply(console, args);
};

const { svg } = await mermaid.render('test-134-graph', src);
console.warn = origConsoleWarn;

// Print relevant logs
for (const log of graphLogs.slice(0, 30)) {
