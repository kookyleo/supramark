...
// Patch graphlib Graph prototype's children() method to log calls
// We need to find the ACTUAL Graph class used by mermaid
// Strategy: intercept through the dagre-d3-es import that mermaid uses
const gl = await import('dagre-d3-es/src/graphlib/index.js');

const origChildren = gl.Graph.prototype.children;
gl.Graph.prototype.children = function(v) {
  const result = origChildren.call(this, v);
  if (v === 'A' && result.length >= 2 && result.includes('B')) {
    console.log(`graph.children(A) = ${JSON.stringify(result)}, nodes() = ${JSON.stringify(origChildren.call(this))}`);
  }
  return result;
};
