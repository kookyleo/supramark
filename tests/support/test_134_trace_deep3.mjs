...
// Track ALL <g> insertions in the nodes section
const allInsertLog = [];
  ...
  for (const mutation of mutations) {
    for (const node of mutation.addedNodes) {
      if (node.nodeType === 1 && node.getAttribute) {
        ...
        if (id.includes('flowchart-') && (cls.includes('node') || cls.includes('cluster'))) {
          allInsertLog.push({ id, cls, type: cls.includes('cluster') ? 'cluster' : 'node' });
        }
      }
    }
  }
...
observer.disconnect();
console.log('All node/cluster insertion order:');
for (const entry of allInsertLog) {
