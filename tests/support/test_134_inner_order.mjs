...
function resolveFont(el) { let size=14,family='sans-serif',bold=false; let node=el; while(node&&node.nodeType===1){ const g=(k)=>(node.getAttribute?node.getAttribute(k):null); const style=node.style??{}; if(!size){const s=g('font-size')??style.fontSize??null;if(s){const m=/([0-9.]+)/.exec(String(s));if(m){size=parseFloat(m[1]);}}} if(!family){const f=g('font-family')??style.fontFamily??null;if(f){family=String(f);}} node=node.parentNode; } return {size,family,bold}; }
...
W.SVGElement.prototype.getBBox=elementBBox;W.SVGElement.prototype.getComputedTextLength=textLen;
...
// Intercept insertBefore to track DOM insert order
const realInsertBefore = W.Element.prototype.insertBefore;
const insertLog = [];
W.Element.prototype.insertBefore = function(newChild, refChild) {
  if (newChild && newChild.getAttribute) {
    const id = newChild.getAttribute('id') || '';
    if (id && id.includes('flowchart-') && (id.includes('-a-') || id.includes('-b-') || id.includes('-c-'))) {
      insertLog.push({ id, parentClass: this.getAttribute('class') || '', time: Date.now() });
    }
  }
  return realInsertBefore.call(this, newChild, refChild);
};
...
const { svg } = await mermaid.render('test-134-dom', src);

console.log('DOM insertBefore order (a, b, c nodes):');
for (const entry of insertLog) {
  console.log(`  ${entry.id} (parent class: ${entry.parentClass})`);
}
