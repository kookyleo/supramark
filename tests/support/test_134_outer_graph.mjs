...
const __ogcs = W.getComputedStyle.bind(W);
W.getComputedStyle = function(el, p) { const cs = __ogcs(el, p); if (!cs||cs.__ps) return cs; const o=cs.getPropertyValue.bind(cs); cs.getPropertyValue=(n)=>{const v=o(n);return v===''||v==null?'0px':v}; try{Object.defineProperty(cs,'__ps',{value:true})}catch{}; return cs; };
...
function textLenFn(){const{size,family,bold}=resolveFont(this);return(this.textContent??'').length*8;}
W.SVGElement.prototype.getBBox=elementBBox; W.SVGElement.prototype.getComputedTextLength=textLenFn;
if(W.HTMLElement)W.HTMLElement.prototype.getBBox=elementBBox; if(W.Element&&!W.Element.prototype.getBBox)W.Element.prototype.getBBox=elementBBox;
function bcr(){return{x:0,y:0,width:0,height:0,top:0,left:0,right:0,bottom:0,toJSON(){return this}};} if(W.HTMLElement)W.HTMLElement.prototype.getBoundingClientRect=bcr; if(W.Element)W.Element.prototype.getBoundingClientRect=bcr;
...
// Patch graphlib to log setNode and setParent calls
const glMod = await import('dagre-d3-es/src/graphlib/index.js');

const origSetNode = glMod.Graph.prototype.setNode;
const origSetParent = glMod.Graph.prototype.setParent;

glMod.Graph.prototype.setNode = function(v, value) {
  const isNew = !Object.prototype.hasOwnProperty.call(this._nodes, v);
  const result = origSetNode.call(this, v, value);
  if (isNew && (v === 'a' || v === 'b' || v === 'c' || v === 'A' || v === 'B' || v === 'O')) {
    const nodes = origSetNode.call(this); // get current nodes
    console.log(`  setNode(${v}) [new] -> nodes now: ${Object.keys(this._nodes).filter(k => !k.startsWith('_'))}`);
  }
  return result;
};

glMod.Graph.prototype.setParent = function(v, parent) {
  const isNew = !Object.prototype.hasOwnProperty.call(this._children, parent) || !this._children[parent][v];
  const result = origSetParent.call(this, v, parent);
  if ((v === 'a' || v === 'b' || v === 'c' || v === 'A' || v === 'B' || v === 'O') && isNew) {
    console.log(`  setParent(${v}, ${parent}) -> children(${parent}): ${Object.keys(this._children[parent] || {}).filter(k => !k.startsWith('_'))}`);
  }
  return result;
};
...
console.log('--- Rendering fixture 134 ---');
