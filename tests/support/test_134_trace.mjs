...
// Monkey-patch insertNode to trace order
const mermaid = (await import('mermaid')).default;
mermaid.initialize({ startOnLoad: false, securityLevel: 'loose', handDrawnSeed: 1 });

// Patch d3 selection to track insert order
const origInsert = W.SVGElement.prototype.insertBefore || function(){};
const insertOrder = [];
const origAppendChild = W.Element.prototype.appendChild;
W.Element.prototype.appendChild = function(child) {
...
const { svg } = await mermaid.render('test-134-trace', src);

console.log('Insert order (appendChild):');
